...
func HttpGetIdentityData(c *gin.Context) {
	logger.HandlerLog.Infof("GetIdentityData is called")
    /// <param name="ueId type: string">UE ID</param>
    /// <param name="appPortId type: AppPortId">Application port identifier (optional)</param>
	...
	// query parameter : basename app-port-id paramName appPortId
	appPortId := queryParams[""]
	// Getting the header params
	ifNoneMatch :=  c.Request.Header["ifNoneMatch"]	
	ifModifiedSince :=  c.Request.Header["ifModifiedSince"]	
	rsp := HandleGetIdentityData( ueId,appPortId,ifNoneMatch,ifModifiedSince, c)
