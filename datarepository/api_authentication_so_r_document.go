...
func HttpCreateAuthenticationSoR(c *gin.Context) {
	logger.HandlerLog.Infof("CreateAuthenticationSoR is called")
    /// <param name="ueId type: string">UE id</param>
    /// <param name="sorData type: SorData"></param>
	...
	// get and parse body
	var sorData SorData
	if err := getDataFromRequestBody(c, &sorData); err != nil {
	...
	rsp := HandleCreateAuthenticationSoR( ueId,sorData,supportedFeatures, c)
...
func HttpQueryAuthSoR(c *gin.Context) {
	logger.HandlerLog.Infof("QueryAuthSoR is called")
    /// <param name="ueId type: string">UE id</param>
    /// <param name="supportedFeatures type: string">Supported Features (optional)</param>
    // Getting the path params
	ueId :=c.Params.ByName("ueId")
	...
	supportedFeatures := queryParams[""]
	rsp := HandleQueryAuthSoR( ueId,supportedFeatures, c)
