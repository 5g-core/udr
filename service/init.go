...
import (
	...
	mongoDBLibLogger "github.com/5g-core/mongodblibrary/logger"
	"github.com/5g-core/http2_util"
	"github.com/5g-core/logger_util"
	openApiLogger "github.com/5g-core/openapi/logger"
	"github.com/5g-core/path_util"
	pathUtilLogger "github.com/5g-core/path_util/logger"
	"github.com/5g-core/udr/consumer"
	udr_context "github.com/5g-core/udr/context"
	"github.com/5g-core/udr/datarepository"
	"github.com/5g-core/udr/factory"
...
func (udr *UDR) Initialize(c *cli.Context) error {
	...
	if config.udrcfg != "" {
		initLog.Infof("UDR config path is : [%s] ", config.udrcfg)
		if err := factory.InitConfigFactory(config.udrcfg); err != nil {
			return err
		}
	} else {
		initLog.Infof("UDR config use default path n5gc/config/udrcfg.yaml ")
...
func (udr *UDR) Start() {
	...
	// Connect to MongoDB
	initLog.Infof("connect mongodb with name [%s] url[%s]", mongodb.Name, mongodb.Url)
	...
	initLog.Infof("UDR bind on : IP[%s] port : [%d]", self.BindingIPv4, self.SBIPort)
	/*
